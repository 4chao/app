<template>
  <view class="u-code-input">
    <view v-for="(item, index) in codeLength" :key="index" class="u-code-input__item" :style="[itemStyle(index)]">
      <view v-if="dot && codeArray.length > index" class="u-code-input__item__dot"></view>
      <text
        v-else
        :style="{
          fontSize: app.addUnit(fontSize),
      ...
      <view v-if="mode === 'line'" class="u-code-input__item__line" :style="[lineStyle]"></view>
    </view>
    <input
...
export default {
  ...
  props: {
    ...
    maxlength: {
      type: [String, Number],
      default: propsDefault.maxlength,
    },
    // 是否用圆点填充
    dot: {
      type: Boolean,
      default: propsDefault.dot,
    },
    // 显示模式，box-盒子模式，line-底部横线模式
    mode: {
      type: String,
      default: propsDefault.mode,
    },
    // 是否细边框
    hairline: {
      type: Boolean,
      default: propsDefault.hairline,
    },
    // 字符间的距离
    space: {
      type: [String, Number],
      default: propsDefault.space,
    },
    // 预置值
    modelValue: {
      type: [String, Number],
      default: propsDefault.value,
    },
    // 是否自动获取焦点
    focus: {
      type: Boolean,
      default: propsDefault.focus,
    },
    // 字体是否加粗
    bold: {
      type: Boolean,
      default: propsDefault.bold,
    },
    // 字体颜色
    color: {
      type: String,
      default: propsDefault.color,
    },
    // 字体大小
    fontSize: {
      type: [String, Number],
      default: propsDefault.fontSize,
    },
    // 输入框的大小，宽等于高
    size: {
      type: [String, Number],
      default: propsDefault.size,
    },
    // 是否隐藏原生键盘，如果想用自定义键盘的话，需设置此参数为true
    disabledKeyboard: {
      type: Boolean,
      default: propsDefault.disabledKeyboard,
    },
    // 边框和线条颜色
    borderColor: {
      type: String,
      default: propsDefault.borderColor,
    },
    // 是否禁止输入"."符号
    disabledDot: {
      type: Boolean,
      default: propsDefault.disabledDot,
    },
  },
  data() {
    return {
  ...
  computed: {
    ...
    lineStyle() {
      ...
      style.width = app.addUnit(this.size)
      // 线条模式下，背景色即为边框颜色
